...
def exact_gradient(
    ...
    return train_state, jax.grad(mll_exact, argnums=2)(
        ds, kernel_fn, model_params
    ), 0, 0.0, 0.0, 0.0
...
def _trace_estimator(
    ...
    noise_scale_grad = model_params.noise_scale * (
        jnp.dot(H_inv_y, H_inv_y) - jnp.sum(left * right) / n_samples
    )
    ...
    def _scan(kernel_params_grad, idx):
        ...
        kernel_params_grad_batch = jax.tree_map(partial(_f, idx=idx), dK_batch)
        kernel_params_grad = jax.tree_map(
            jnp.add, kernel_params_grad, kernel_params_grad_batch
        )
        return kernel_params_grad, None

    xs = jnp.reshape(jnp.arange(x.shape[0]), (-1, batch_size))
    kernel_params_grad, _ = jax.lax.scan(
        _scan, jax.tree_map(jnp.zeros_like, model_params.kernel_params), xs
    )
...
def standard_gradient(
    ...
    model_params_grad = _trace_estimator(
        v[:, 0], v[:, 1:], b[:, 1:], model_params, ds, kernel_grad_fn
    )

    train_state = TrainState(
        key=key, v0=v, z=train_state.z, feature_params=None, w=None, eps=None
    )
...
def pathwise_gradient(
    ...
    v0, b, key, feature_params, w, eps = _pathwise_init(
        train_state, model_params, ds.x, ds.y, feature_params_fn, n_features, warm_start, batch_size
    )
    ...
    model_params_grad = _trace_estimator(
        v[:, 0], v[:, 1:], v[:, 1:], model_params, ds, kernel_grad_fn
    )

    train_state = TrainState(
        key=key, v0=v, z=None, feature_params=feature_params, w=w, eps=eps
    )
    return train_state, model_params_grad, solver_iters, solver_time, r_norm_y, r_norm_z
...
def _pathwise_init(
    ...
    f0 = feature_vec_prod(x, model_params.kernel_params, feature_params, w, batch_size)
