...
def fit(
    ...
    if checkpoint is None:
            ...
            if cfg.warm_start:
                if cfg.estimator_name == "standard":
                    ...
                    if cfg.pathwise_init:
                        train_state = jnp.load("./download/pathwise_init.npy", allow_pickle=True).item()[cfg.dataset_name][cfg.dataset_split]
                    else:
        ...
        train_state = jax.tree_map(lambda x: float_cast(x) if isinstance(x, Array) and jnp.issubdtype(x, jnp.floating) else x, train_state)
    ...
    def log(i, model_params_u, train_time, solver_time, solver_iters, r_norm_y, r_norm_z, commit=True):
        ...
        if cfg.log_wandb:
            model_params_dict = {
                ...
                **{
                    f"params/length_scale_{i}": l
                    for i, l in enumerate(model_params.kernel_params.length_scale)
                },
            }
